...
由于 Wbi 接口有相关签名校验，为了方便开发者测试，这里给出 Apifox 前置脚本方便直接导入测试
...
function getWbiKeys() {
  pm.sendRequest("https://api.bilibili.com/x/web-interface/nav", function (err, response) {
    const { data: { wbi_img: { img_url, sub_url } } } = response.json();
...
if (img_key == undefined || sub_key == undefined || wbi_key_expire == undefined || wbi_key_expire < Math.round(Date.now()/1000)){
  getWbiKeys();
