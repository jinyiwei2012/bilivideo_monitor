...
class PoissonRegressionAlgorithm(BaseAlgorithm):
    ...
    def _fit_poisson(self, X: np.ndarray, y: np.ndarray,
                     max_iter: int = 200, tol: float = 1e-6) -> Tuple[np.ndarray, float]:
    ...
    def predict(self, video_data: Dict[str, Any],
        ...
        if remaining <= 0:
            return PredictionResult(
                ...
                metadata={'method': 'poisson'}, timestamp=datetime.now()
            )
        ...
        if len(views_vals) < 6:
            ...
            return PredictionResult(
                ...
                confidence=0.3, current_views=current_views,
                current_velocity=velocity,
                metadata={'method': 'poisson_fallback'}, timestamp=datetime.now()
            )
        ...
        try:
            ...
            if len(X) < 3:
                ...
                return PredictionResult(
                    ...
                    confidence=0.3, current_views=current_views,
                    current_velocity=velocity,
                    metadata={'method': 'poisson_insufficient'}, timestamp=datetime.now()
                )
