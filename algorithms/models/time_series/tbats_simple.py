...
三角函数处理多重季节性，Box-Cox变换后分解趋势与季节，残差拟合ARMA
"""
...
class TbatsSimpleAlgorithm(BaseAlgorithm):
    ...
    description = "三角函数多重季节+Box-Cox+ARMA"
    category = "时间序列"
    default_weight = 1.2
    ...
    def predict(self, video_data: Dict[str, Any], threshold: int = 100000) -> PredictionResult:
        ...
        try:
            ...
            for p in periods:
                if p > n // 2:
                    continue
                future_seasonal += (amp_cos * np.cos(2 * np.pi * future_t / p) +
                                    amp_sin * np.sin(2 * np.pi * future_t / p))
