...
class _EntryRow:
    ...
    def __init__(self, parent, bvid: str, period: str, existing: dict = None):
        ...
        frame = tk.Frame(parent, bg=C["bg_surface"],
                         highlightthickness=1, highlightbackground=C["border_sub"])
        frame.pack(fill=tk.X, pady=2, ipady=2)

        tk.Label(frame, text=bvid, bg=C["bg_surface"], fg=C["accent"],
                 font=FONT_BOLD, width=14, anchor="w"
                 ).grid(row=0, column=0, padx=(6, 4))
        tk.Label(frame, text=period, bg=C["bg_surface"],
                 fg=PERIOD_COLORS[period], font=FONT_BOLD, width=4, anchor="w"
...
class MilestoneStatsWindow:
    ...
    def __init__(self, parent=None,
        ...
        self.dlg = DialogBase(parent, "投稿里程碑 — 一周 / 月 / 年后数据",
                              "1200x760", resizable=(True, True), modal=True)
    ...
    def _setup_ui(self):
        ...
        nb.bind("<<NotebookTabChanged>>",
                lambda e: self._reload_comparison()
    ...
    def _build_entry_tab(self):
        ...
        # 顶部卡片
        top = tk.Frame(tab, bg=C["bg_elevated"],
                       highlightthickness=1, highlightbackground=C["border_sub"])
        ...
        for p in PERIODS:
            ...
            tk.Checkbutton(period_col, text=p, variable=var,
                           bg=C["bg_elevated"], fg=PERIOD_COLORS[p],
        ...
        ttk.Button(btn_col, text="💾 保存全部", style="Primary.TButton",
        ...
        cols = ["BV号", "周期", "播放量*", "点赞数", "投币数", "分享数",
                "收藏数", "弹幕数", "评论数", "备注"]
        widths = [14, 4, 9, 9, 9, 9, 9, 9, 9, 20]
        x = 6
        for ct, w in zip(cols, widths):
            tk.Label(header, text=ct, bg=C["bg_surface"], fg=C["text_3"],
                     font=FONT_SM, width=w, anchor="w").place(x=x, y=3)
            x += w * 7
    ...
    def _save_all(self):
        ...
        if errors:
            msg += f"，失败 {errors} 条"
        self._entry_status.config(text=msg,
                                  fg=C["success"] if not errors else C["warning"])
        if saved:
            self._reload_comparison()
    ...
    def _build_compare_tab(self):
        ...
        tk.Label(ctrl, text="展示指标：", bg=C["bg_surface"],
                 fg=C["text_2"], font=FONT).pack(side=tk.LEFT)
        self._metric_var = tk.StringVar(value="view_count")
        for label, val in [("播放量", "view_count"), ("点赞数", "like_count"),
        ...
        tk.Label(ff, text="筛选视频：", bg=C["bg_base"], fg=C["text_2"],
                 font=FONT_SM).pack(side=tk.LEFT)
        self._filter_entry = tk.Entry(ff, font=FONT_SM, width=60,
                                      bg=C["bg_elevated"], fg=C["text_1"],
                                      insertbackground=C["text_1"],
                                      relief="flat", bd=1)
        self._filter_entry.pack(side=tk.LEFT, padx=4)
        tk.Label(ff, text="（BV号关键词，逗号分隔）",
                 bg=C["bg_base"], fg=C["text_3"], font=FONT_SM).pack(side=tk.LEFT)
        ttk.Button(ff, text="应用筛选",
        ...
        tk.Label(tbl_frame, text="明细数据", bg=C["bg_base"], fg=C["text_3"],
                 font=FONT_SM).pack(anchor="w")
        ...
        self._cmp_status = tk.Label(tab, text="", bg=C["bg_base"],
    ...
    def _fill_table(self):
        ...
        for bvid, periods in sorted(self._all_data.items()):
            ...
            self._tbl.insert("", tk.END, iid=bvid, values=(
                bvid, title,
                _v("1周", "view_count"), _v("1月", "view_count"),
                _v("1年", "view_count"),
                _v("1周", "like_count"), _v("1月", "like_count"),
                _v("1年", "like_count"), latest,
            ))
    ...
    def _redraw_compare(self):
        ...
        if ft:
            ...
            data = {bv: pd for bv, pd in self._all_data.items()
                    if any(k.upper() in bv.upper() for k in ks)}
        else:
            data = self._all_data

        if not data:
            c.create_text((c.winfo_width() or 600)//2, (c.winfo_height() or 300)//2,
        ...
        for i in range(6):
            ...
            c.create_text(ML - 6, y,
                          text=f"{val/10000:.0f}w" if val >= 10000 else str(int(val)),
                          anchor="e", fill=C["text_2"], font=("Consolas", 8))
                ...
                if val:
                    ...
                    if by < MT + ch - 14:
                        c.create_text((bx + bx2) / 2, by - 4,
                                      text=f"{val/10000:.0f}w" if val >= 10000 else str(int(val)),
                                      anchor="s", fill=color, font=("Consolas", 7, "bold"))
                else:
                    c.create_rectangle(bx, MT + ch - 4, bx2, MT + ch, fill=C["border"], outline="")
            lx = gx + bar_total_w / 2
            c.create_text(lx, MT + ch + 6, text=title, anchor="n",
                          fill=C["text_1"], font=("Microsoft YaHei UI", 8))
            c.create_text(lx, MT + ch + 22, text=bv, anchor="n",
                          fill=C["text_3"], font=("Consolas", 7))

        lgx = ML + 6
        for p in PERIODS:
            c.create_rectangle(lgx, MT + ch + 52, lgx + 10, MT + ch + 62,
                               fill=PERIOD_COLORS[p], outline="")
            c.create_text(lgx + 14, MT + ch + 57, text=f"投稿{p}后",
                          anchor="w", fill=PERIOD_COLORS[p], font=("Microsoft YaHei UI", 8))
            lgx += 90

        ml = next((lb for key, lb, *_ in FIELDS if key == metric), metric)
        c.create_text(ML + cw // 2, 14,
                      text=f"投稿里程碑对比 — {ml}",
                      fill=C["text_1"], font=("Microsoft YaHei UI", 10, "bold"), anchor="n")
