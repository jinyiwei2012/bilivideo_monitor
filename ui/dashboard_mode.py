...
class DashboardWindow:
    ...
    def _setup_ui(self):
        ...
        tk.Label(self._header, text="📊 数据大屏",
                 bg=_DASH_COLORS["bg"], fg=_DASH_COLORS["text_1"],
                 font=("Microsoft YaHei UI", 20, "bold")).pack(side=tk.LEFT)
        self._time_lbl = tk.Label(self._header, text="",
                                   bg=_DASH_COLORS["bg"], fg=_DASH_COLORS["text_2"],
        ...
        for i in range(self._total_pages):
            d = tk.Label(self._dots, text="●", bg=_DASH_COLORS["bg"],
    ...
    def _build_overview(self):
        ...
        for label, val, color in cards:
            c = tk.Frame(stat_row, bg=_DASH_COLORS["card_bg"],
                         highlightthickness=1,
                         highlightbackground="#30363d")
            c.pack(side=tk.LEFT, fill=tk.BOTH, expand=True, padx=8, ipady=30)
            tk.Label(c, text=label, bg=_DASH_COLORS["card_bg"],
                     fg=_DASH_COLORS["text_2"],
                     font=("Microsoft YaHei UI", 12)).pack()
            tk.Label(c, text=val, bg=_DASH_COLORS["card_bg"],
                     fg=color,
                     font=("Consolas", 28, "bold")).pack(pady=(8, 0))

        # 最近视频列表
        titles = ["视频列表"]
        cols = ["BV号", "标题", "播放", "点赞", "弹幕"]
        table_frame = tk.Frame(self._content, bg=_DASH_COLORS["card_bg"],
                                highlightthickness=1,
                                highlightbackground="#30363d")
        table_frame.pack(fill=tk.BOTH, expand=True, padx=8, pady=8, ipady=10)
        tk.Label(table_frame, text=titles[0], bg=_DASH_COLORS["card_bg"],
                 fg=_DASH_COLORS["text_2"],
        ...
        for v in videos[:8]:
            ...
            tk.Label(row, text=v.get("bvid", "")[:12], bg=_DASH_COLORS["card_bg"],
                     fg=_DASH_COLORS["accent"], font=("Consolas", 9), width=14).pack(side=tk.LEFT)
            tk.Label(row, text=v.get("title", "")[:30], bg=_DASH_COLORS["card_bg"],
                     fg=_DASH_COLORS["text_1"], font=("Microsoft YaHei UI", 9), width=35, anchor="w").pack(side=tk.LEFT)
            tk.Label(row, text=_fmt(v.get("view_count", 0)), bg=_DASH_COLORS["card_bg"],
                     fg=_DASH_COLORS["text_2"], font=("Consolas", 9), width=12, anchor="e").pack(side=tk.LEFT)
            tk.Label(row, text=_fmt(v.get("like_count", 0)), bg=_DASH_COLORS["card_bg"],
                     fg=_DASH_COLORS["text_2"], font=("Consolas", 9), width=10, anchor="e").pack(side=tk.LEFT)
            tk.Label(row, text=_fmt(v.get("danmaku_count", 0)), bg=_DASH_COLORS["card_bg"],
                     fg=_DASH_COLORS["text_2"], font=("Consolas", 9), width=10, anchor="e").pack(side=tk.LEFT)

    # ── 第2页：排行 ────────────────────────────
    def _build_ranking(self):
        videos = sorted(self.gui.monitored_videos,
                        key=lambda v: v.get("view_count", 0), reverse=True)
        if not videos:
            tk.Label(self._content, text="暂无数据",
                     bg=_DASH_COLORS["bg"], fg=_DASH_COLORS["text_2"],
                     font=("Microsoft YaHei UI", 16)).pack()
            return

        tk.Label(self._content, text="🏆 播放量排行 Top10",
                 bg=_DASH_COLORS["bg"], fg=_DASH_COLORS["text_1"],
        ...
        c = tk.Canvas(chart_frame, bg=_DASH_COLORS["bg"], width=canvas_w,
        ...
        for i, v in enumerate(top):
            ...
            c.create_rectangle(10, y + 6, 10 + bar_w, y + bar_h - 6,
                               fill=_DASH_COLORS["accent"], outline="",
                               stipple="" if i < 3 else "gray50")
            c.create_text(16, y + bar_h // 2, text=f"#{i+1}",
                          fill="white", font=("Consolas", 10, "bold"), anchor="w")
            c.create_text(24 + bar_w, y + bar_h // 2,
    ...
    def _build_prediction(self):
        tk.Label(self._content, text="🎯 预测总览",
                 bg=_DASH_COLORS["bg"], fg=_DASH_COLORS["text_1"],
                 font=("Microsoft YaHei UI", 14, "bold")).pack(anchor="w")
        ...
        for v in self.gui.monitored_videos[:6]:
            views = v.get("view_count", 0)
            card = tk.Frame(row, bg=_DASH_COLORS["card_bg"],
                            highlightthickness=1,
                            highlightbackground="#30363d")
            card.pack(side=tk.LEFT, fill=tk.BOTH, expand=True, padx=6, ipady=14)

            tk.Label(card, text=v.get("title", "")[:18],
                     bg=_DASH_COLORS["card_bg"], fg=_DASH_COLORS["text_1"],
                     font=("Microsoft YaHei UI", 10, "bold")).pack(anchor="w", padx=10, pady=(8, 2))
            tk.Label(card, text=f"播放: {_fmt(views)}",
                     bg=_DASH_COLORS["card_bg"], fg=_DASH_COLORS["text_2"],
            ...
            for t, name in zip(THRESHOLDS, THRESHOLD_NAMES):
                ...
                tk.Label(card, text=f"  {name}: {status}",
    ...
    def _build_health(self):
        tk.Label(self._content, text="💚 健康概览",
                 bg=_DASH_COLORS["bg"], fg=_DASH_COLORS["text_1"],
                 font=("Microsoft YaHei UI", 14, "bold")).pack(anchor="w")
        ...
        alert_frame = tk.Frame(self._content, bg=_DASH_COLORS["card_bg"],
                                highlightthickness=1,
                                highlightbackground="#30363d")
        alert_frame.pack(fill=tk.BOTH, expand=True, pady=10, ipady=16)

        tk.Label(alert_frame, text="实时预警",
                 bg=_DASH_COLORS["card_bg"], fg=_DASH_COLORS["text_2"],
                 font=("Microsoft YaHei UI", 10, "bold"),
                 anchor="w").pack(fill=tk.X, padx=12, pady=(8, 4))
        ...
        try:
            ...
            for v in self.gui.monitored_videos[:6]:
                try:
                    ...
                    grade_colors = {"S": _DASH_COLORS["bilibili"], "A": _DASH_COLORS["accent"],
                    ...
                    row.pack(fill=tk.X, padx=12, pady=1)
                    tk.Label(row, text=v.get("title", "")[:20],
                             bg=_DASH_COLORS["card_bg"], fg=_DASH_COLORS["text_1"],
                             font=("Microsoft YaHei UI", 9)).pack(side=tk.LEFT)
