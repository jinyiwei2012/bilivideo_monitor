...
def _build_export_row(index: int, row, extra: dict = None, algo_names: list = None) -> list:
    ...
    base_row = [
        ...
        row['share_count'], row['favorite_count'], row['danmaku_count'],
        row['reply_count'],
        row.get('viewers_app', '') or '', row.get('viewers_web', '') or '',
        row.get('viewers_total', '') or '', row.get('like_view_ratio', '') or '',
...
class DatabaseQueryWindow:
    ...
    def setup_ui(self):
        ...
        tk.Label(fr, text="视频筛选", bg=C["bg_elevated"], fg=C["text_2"],
                 font=FONT, width=12, anchor="w").pack(side=tk.LEFT)
        self.video_filter_var = tk.StringVar(value="全部视频")
        self.video_filter_combo = ttk.Combobox(fr, textvariable=self.video_filter_var,
        ...
        tk.Label(mr, text="查询方式", bg=C["bg_elevated"], fg=C["text_2"],
                 font=FONT, width=12, anchor="w").pack(side=tk.LEFT)
        self.query_mode = tk.StringVar(value="latest")
        mode_combo = ttk.Combobox(mr, textvariable=self.query_mode,
        ...
        self._query_btn = ttk.Button(br, text="查询", command=self._do_query,
        ...
        tk.Label(res_label, text="查询结果", bg=C["bg_base"], fg=C["text_2"],
                 font=("Microsoft YaHei UI", 8, "bold")).pack(side=tk.LEFT)
        self.status_var = tk.StringVar(value="就绪")
        tk.Label(res_label, textvariable=self.status_var, bg=C["bg_base"],
                 fg=C["text_3"], font=FONT_SM).pack(side=tk.RIGHT)

        res_frame = tk.Frame(container, bg=C["bg_elevated"],
                             highlightthickness=1, highlightbackground=C["border_sub"])
        res_frame.pack(fill=tk.BOTH, expand=True, pady=(4, 0))

        columns = ('seq', 'bv', 'timestamp', 'views', 'likes', 'coins', 'shares',
                   'favorites', 'danmaku', 'reply', 'viewers_total',
                   'viewers_web', 'viewers_app', 'like_ratio')
        self.result_tree = ttk.Treeview(res_frame, columns=columns,
    ...
    def _load_extra_data(self, bvid: str, timestamp: str) -> dict:
        ...
        try:
            ...
            cur = conn.cursor()
            cur.execute('SELECT * FROM predictions WHERE created_at <= ? ORDER BY algorithm, created_at DESC', (timestamp,))
            ...
            cur.execute('SELECT * FROM weekly_scores WHERE timestamp <= ? ORDER BY timestamp DESC LIMIT 1', (timestamp,))
            ws = cur.fetchone()
            if ws:
                wd = dict(ws)
                for k in ['total_score', 'view_score', 'interaction_score', 'favorite_score',
                          'coin_score', 'like_score', 'correction_a', 'correction_b',
                          'correction_c', 'correction_d', 'base_view_score']:
                    extra[f'weekly_{k}'] = wd.get(k, '')
            cur.execute('SELECT * FROM yearly_scores WHERE timestamp <= ? ORDER BY timestamp DESC LIMIT 1', (timestamp,))
            ys = cur.fetchone()
            if ys:
                yd = dict(ys)
                for k in ['total_score', 'view_score', 'interaction_score', 'favorite_score',
    ...
    def _on_mode_change(self):
        ...
        if mode == '最新N条':
            tk.Label(self.param_frame, text="数量N:", bg=C["bg_elevated"],
                     fg=C["text_2"], font=FONT).pack(side=tk.LEFT)
            self.param_var = tk.StringVar(value="100")
            ttk.Entry(self.param_frame, textvariable=self.param_var,
                      width=10, font=FONT).pack(side=tk.LEFT, padx=(4, 0))
            tk.Label(self.param_frame, text="(要查询的记录数)", bg=C["bg_elevated"],
                     fg=C["text_3"], font=FONT_SM).pack(side=tk.LEFT, padx=8)
        elif mode == '播放首次大于X':
            tk.Label(self.param_frame, text="播放量X:", bg=C["bg_elevated"],
                     fg=C["text_2"], font=FONT).pack(side=tk.LEFT)
            self.param_var = tk.StringVar(value="10000")
            ttk.Entry(self.param_frame, textvariable=self.param_var,
                      width=15, font=FONT).pack(side=tk.LEFT, padx=(4, 0))
        elif mode == '播放趋势':
            tk.Label(self.param_frame, text="选择视频:", bg=C["bg_elevated"],
                     fg=C["text_2"], font=FONT).pack(side=tk.LEFT)
            self.video_combo = ttk.Combobox(self.param_frame,
                                            textvariable=self.video_combo_var,
                                            state='readonly', width=30, font=FONT)
            self.video_combo.pack(side=tk.LEFT, padx=(4, 0))
        elif mode == '全量数据':
            tk.Label(self.param_frame, text="(将导出所有监控记录数据)", bg=C["bg_elevated"],
        ...
        def _worker():
            ...
            try:
                ...
                if mode == '最新N条':
                    ...
                    if filter_bvid:
                        cur.execute('SELECT * FROM monitor_records WHERE bvid = ? ORDER BY timestamp DESC LIMIT ?', (filter_bvid, limit))
                    else:
                    ...
                    if filter_bvid:
                        cur.execute('SELECT * FROM monitor_records WHERE bvid = ? AND view_count > ? ORDER BY timestamp ASC LIMIT 1', (filter_bvid, thr))
                    else:
                        cur.execute('''WITH fa AS (SELECT bvid, MIN(timestamp) as ft FROM monitor_records WHERE view_count > ? GROUP BY bvid)
                                       SELECT m.* FROM monitor_records m INNER JOIN fa f ON m.bvid = f.bvid AND m.timestamp = f.ft ORDER BY m.timestamp DESC''', (thr,))
                elif mode == '播放趋势':
    ...
    def _finish_query(self, raw_rows, extra_list, algo_names):
        ...
        for i, row in enumerate(raw_rows, 1):
            ...
            vals = (i, row['bvid'], row['timestamp'],
                    f"{row['view_count']:,}", f"{row['like_count']:,}",
                    f"{row['coin_count']:,}", f"{row['share_count']:,}",
                    f"{row['favorite_count']:,}", f"{row['danmaku_count']:,}",
                    f"{row['reply_count']:,}", f"{(row.get('viewers_total') or 0):,}",
                    f"{(row.get('viewers_web') or 0):,}",
                    f"{(row.get('viewers_app') or 0):,}", f"{lr:.4f}")
    ...
    def _get_export_default_name(self, ext: str) -> str:
        ...
        mn = {
            '最新N条': 'latest', '播放首次大于X': f'above{self.param_var.get()}',
    ...
    def _export_csv(self):
        if not self.query_results:
            ...
            return
        fp = filedialog.asksaveasfilename(title="导出CSV", defaultextension=".csv",
    ...
    def _export_excel(self):
        ...
        try:
            ...
            messagebox.showerror("错误", "需要安装 openpyxl 库\n请运行: pip install openpyxl",
                                parent=self.window)
            return
        fp = filedialog.asksaveasfilename(title="导出Excel", defaultextension=".xlsx",
