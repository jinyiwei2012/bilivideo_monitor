...
class DatabaseQueryWindow(DialogBase):
    ...
    def _setup_ui(self):
        ...
        self._status_display = QLabel("天依就绪啦 ♪")
    ...
    def _do_query(self):
        """启动查询"""
        if not os.path.exists(self.db_path):
            QMessageBox.critical(self, "呜…出错了", "呜…数据库文件找不到啦，像弄丢的一页歌谱，天依会帮你找回来的 ♪")
            return
        ...
        if mode == "播放趋势":
            ...
            if not sel:
                if filter_bvid:
                    ...
                    QMessageBox.warning(self, "要注意哦…", "要先选一个视频哦，天依才能听见它的旋律呢…♪")
                    return
            else:
            ...
            if not is_valid_bvid(bvid_for_trend):
                QMessageBox.critical(self, "呜…出错了", "呜…这个BV号像走调的音符呢，天依帮你再看看格式哦 ♪")
                return
        ...
        self._extra_data = []
        self._algo_names = []
        self._update_status("天依正在翻找数据的乐谱哦…♪")
    ...
    def _delete_selected(self):
        ...
        if not sel:
            QMessageBox.warning(self, "要注意哦…", "要先选中要删除的记录哦，天依不想误删任何一段旋律呢…♪")
            return
        ...
        if source_bvid:
            ...
            if not db_path:
                QMessageBox.critical(self, "呜…出错了", "呜…视频独立库文件不见啦，像丢在风里的乐谱，天依再找找哦 ♪")
                return
        else:
        ...
        self._update_status(f"天依正在轻轻擦去 {len(sel)} 条记录哦…♪")
