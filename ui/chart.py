...
def draw_chart_annotations(c, history, views_list, px, py, W, H, ML, MR, MB):
    ...
    c.create_text(lx, lv - 14, text=fmt_num(views_list[-1]), fill="#ffffff", font=("Consolas", 8, "bold"))
    ...
    items = [("播放量", C["bilibili"])] + [(THRESHOLD_NAMES[i] + "阈值", THRESH_COLORS[i]) for i in range(3)]
    lx0 = ML + 4
    ...
    c.create_text(W - MR - 2, 12, text=f"{len(history)} 个数据点", anchor="e", fill=C["text_3"], font=("Consolas", 8))
...
def draw_chart_grid(c, W, H, ML, MR, MT, MB, cw, ch, min_v, max_v):
    ...
    for i in range(rows + 1):
        ...
        c.create_text(ML - 4, y, text=abbrev(val), anchor="e", fill=C["text_3"], font=("Consolas", 8))
