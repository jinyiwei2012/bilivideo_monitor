...
class PredictionPanel:
    ...
    def _build_pred_hero(self, weighted_pred, current_views, rate_per_sec):
        ...
        ctk.CTkLabel(outer, text=delta_text, text_color=delta_color, font=FONT, fg_color="transparent").pack(anchor="w")
        if rate_per_sec > 0:
            ...
            ctk.CTkLabel(outer, text=rate_str, text_color=C["accent"], font=FONT_SM, fg_color="transparent").pack(
                anchor="w", pady=(2, 0)
            )
    ...
    def _update_algo_list(self, results, failed):
        ...
        if results:
            ...
            for i, (name, pred, weight, conf, predicted_hours) in enumerate(results):
                ...
                ctk.CTkLabel(top_row, text="●", text_color=dot_c, font=FONT_SM, fg_color="transparent").pack(
                    side=tk.LEFT
                )
                ctk.CTkLabel(
                    top_row, text=" " + name[:18], text_color=C["text_1"], font=FONT, fg_color="transparent"
                ).pack(side=tk.LEFT)
                ctk.CTkLabel(
                    top_row,
                    text=fmt_num(pred),
