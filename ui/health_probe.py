...
from utils.interaction_quality import (
    calculate_probe, calculate_probe_from_dict,
    ProbeResult, format_probe_result,
)
...
class HealthProbeWindow:
    ...
    def _setup_ui(self):
        ...
        self.dlg.header(f"一键三连健康探针 — {title}",
                        "基于点赞率·硬币率·收藏率·分享率的综合评估")
        ...
        # 右侧摘要卡片
        summary = tk.Frame(top, bg=C["bg_elevated"], highlightthickness=1,
                           highlightbackground=C["border_sub"])
        ...
        # 大分数
        grade_colors = {"S": "#fb7299", "A": "#23ade5", "B": "#42b983",
                        "C": "#f5a623", "D": "#e74c3c"}
        gc = grade_colors.get(grade, C["text_1"])
        tk.Label(summary, text=f"{score:.0f}", bg=C["bg_elevated"], fg=gc,
                 font=("Consolas", 48, "bold")).pack(anchor="center", pady=(10, 0))
        tk.Label(summary, text=f"评级 {grade}", bg=C["bg_elevated"], fg=gc,
                 font=("Microsoft YaHei UI", 16, "bold")).pack(anchor="center")
        tk.Label(summary, text=f"{len(self.result.anomalies)} 项异常",
                 bg=C["bg_elevated"],
                 fg=C["danger"] if self.result.anomalies else C["success"],
                 font=("Microsoft YaHei UI", 10)).pack(anchor="center", pady=(6, 0))
        ...
        for label, val, unit in items:
            ...
            tk.Label(sf, text=label, bg=C["bg_elevated"], fg=C["text_2"],
                     font=("Microsoft YaHei UI", 9), width=8, anchor="w").pack(side=tk.LEFT)
            tk.Label(sf, text=f"{val:.2f}{unit}", bg=C["bg_elevated"], fg=C["text_1"],
        ...
        if self.result.anomalies:
            ...
            for a in self.result.anomalies:
                tk.Label(sec, text=f"  • {a}", bg=C["bg_elevated"], fg=C["danger"],
                         font=("Microsoft YaHei UI", 9), anchor="w").pack(fill=tk.X, pady=1)

        if self.result.tips:
            sec2 = self.dlg.section(title="💡 建议", parent=bottom, padding=8)
            for t in self.result.tips:
                tk.Label(sec2, text=f"  • {t}", bg=C["bg_elevated"], fg=C["text_1"],
                         font=("Microsoft YaHei UI", 9), anchor="w").pack(fill=tk.X, pady=1)
    ...
    def _draw_radar(self):
        ...
        for ang, lbl in zip(angles, labels):
            ...
            c.create_line(cx, cy, cx + radius * math.cos(ang),
                          cy + radius * math.sin(ang),
                          fill=C["border_sub"], width=1)
            c.create_text(x2, y2, text=lbl, fill=C["text_2"],
        ...
        for val, ang in zip(values, angles):
            ...
            pts.extend([cx + r * math.cos(ang), cy + r * math.sin(ang)])
        c.create_polygon(*pts, fill="#fb729944", outline="#fb7299",
                         width=2)
