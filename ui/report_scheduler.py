...
class ReportSchedulerWindow:
    ...
    def _setup_ui(self):
        ...
        tk.Label(row, text="导出格式:", bg=C["bg_elevated"], fg=C["text_2"],
                 font=("Microsoft YaHei UI", 10)).pack(side=tk.LEFT)
        self._format_var = tk.StringVar(value="html")
        tk.Radiobutton(row, text="HTML", variable=self._format_var,
                       value="html", bg=C["bg_elevated"]).pack(side=tk.LEFT, padx=6)
        tk.Radiobutton(row, text="Excel", variable=self._format_var,
                       value="excel", bg=C["bg_elevated"]).pack(side=tk.LEFT, padx=6)
        tk.Radiobutton(row, text="两者", variable=self._format_var,
                       value="both", bg=C["bg_elevated"]).pack(side=tk.LEFT, padx=6)

        ttk.Button(sec, text="立即导出", command=self._export_now,
                   style="Primary.TButton").pack(anchor="w", padx=4, pady=(8, 0))

        self._export_status = tk.Label(sec, text="", bg=C["bg_elevated"],
        ...
        tk.Label(sec2, text="定时导出功能将在后续版本中支持。\n目前可使用「立即导出」手动生成报告。",
                 bg=C["bg_elevated"], fg=C["text_3"],
                 font=("Microsoft YaHei UI", 9), anchor="w",
                 justify="left").pack(fill=tk.X, padx=4, pady=4)

        # 已导出文件列表
        sec3 = self.dlg.section(title="已导出文件", padding=8)
        self._file_list = tk.Listbox(sec3, bg=C["bg_base"], fg=C["text_1"],
        ...
        ttk.Button(btn_row, text="🔄 刷新列表",
                   command=self._refresh_file_list).pack(side=tk.LEFT, padx=2)
