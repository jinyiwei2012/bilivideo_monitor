...
class TrendTab:
    ...
    def _build(self):
        ...
        for key, label in METRICS:
            ttk.Radiobutton(right, text=label, variable=self._metric,
                            value=key, command=self._draw).pack(anchor="w")

        ttk.Button(right, text="开始对比", command=self._start).pack(
            pady=(10,0), fill=X)
        ...
        self._canvas = tk.Canvas(mid, bg=C.get("canvas_bg","#0d1117"),
        ...
        # 初始提示
        self._canvas.after(100, lambda: self._canvas.create_text(
    ...
    def _start(self):
        ...
        self._selected = [self._monitored_videos[i]
                                 for i in sel if i < len(self._monitored_videos)]
