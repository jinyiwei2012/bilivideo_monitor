...
class SettingsNotificationMixin:
    ...
    def _build_notification_tab(self, nb):
        ...
        self.onebot_token = self._field(
            sec, "Access Token", self._cfg.get("onebot", {}).get("access_token", ""), show_password=True
        )
    ...
    def _build_webhook_section(self, page):
        ...
        for wh in saved:
            if isinstance(wh, dict) and wh.get("url"):
                self._add_webhook_row(wh.get("name", ""), wh.get("type", "generic"), wh.get("url", ""))
        ...
        hint = QLabel(
            "支持 企业微信/钉钉/Slack/Discord/自定义。填入机器人 Webhook 地址后,阈值突破、告警、报告都会同步推送 ♪ (仅限公网 http/https 地址,内网/回环地址会被安全策略拦截)"
        )
    ...
    def _test_connection(self):
        ...
        def _do_test():
            try:
                result = notification_manager.test_connection(http_url=http_url, ws_url=ws_url, token=token)
                from PyQt6.QtCore import QTimer
