...
class TrendingDiscoveryWindow:
    ...
    def _setup_ui(self):
        ...
        for label, key in tabs:
            b = tk.Label(tab_bar, text=label, bg=C["bg_surface"], fg=C["text_2"],
        ...
        ttk.Button(refresh_frame, text="🔄 刷新", command=self._load_popular,
                   style="Primary.TButton").pack(side=tk.RIGHT)
        # Re-pack the first tab
        self._tab_btns["popular"].pack(side=tk.LEFT)

        # 视频列表区
        list_frame = tk.Frame(self.dlg.container, bg=C["bg_elevated"],
        ...
        self._canvas = tk.Canvas(sc, bg=C["bg_elevated"],
    ...
    def _load_popular(self):
        if not self.api:
            return
        self._clear_cards()
        tk.Label(self._card_frame, text="加载中...",
    ...
    def _display_videos(self, videos: List[Dict]):
        ...
        for v in videos:
            ...
            top.pack(fill=tk.X)
            tk.Label(top, text=title, bg=C["bg_surface"], fg=C["text_1"],
                     font=("Microsoft YaHei UI", 10), anchor="w",
                     wraplength=500, justify="left").pack(side=tk.LEFT, fill=tk.X, expand=True)

            if bvid:
                ttk.Button(top, text="+ 监控",
                           command=lambda b=bvid: self._add_monitor(b)
                           ).pack(side=tk.RIGHT, padx=(8, 0))

            meta = tk.Frame(card, bg=C["bg_surface"])
            meta.pack(fill=tk.X, pady=(4, 0))
            tk.Label(meta, text=f"👤 {author}", bg=C["bg_surface"], fg=C["text_2"],
                     font=("Microsoft YaHei UI", 9)).pack(side=tk.LEFT, padx=(0, 12))
            tk.Label(meta, text=f"▶ {self._fmt(views)}", bg=C["bg_surface"], fg=C["text_2"],
                     font=("Microsoft YaHei UI", 9)).pack(side=tk.LEFT, padx=(0, 12))
            tk.Label(meta, text=f"👍 {self._fmt(likes)}", bg=C["bg_surface"], fg=C["text_2"],
                     font=("Microsoft YaHei UI", 9)).pack(side=tk.LEFT, padx=(0, 12))
            tk.Label(meta, text=f"💬 {self._fmt(danmaku)}", bg=C["bg_surface"], fg=C["text_2"],
